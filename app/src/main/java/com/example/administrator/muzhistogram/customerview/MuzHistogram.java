package com.example.administrator.muzhistogram.customerview;
...
import com.example.administrator.muzhistogram.R;

import java.util.ArrayList;

/**
 * Created by Muz on 2018/1/8.
 * 柱状图
 *
 * 当不超出屏幕或者到达边缘停止滑动问题 带实现
 */

public class MuzHistogram extends View {
//    //画笔
    Paint paint;
    ...
    public MuzHistogram(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(context,attrs);
    }

    public MuzHistogram(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public MuzHistogram(Context context) {
        this(context, null);
    }
int initHeight;
    @Override
...
//        paint.setColor(colours.get(0));
        ...
        TypedArray ta=getContext().obtainStyledAttributes(attrs, R.styleable.MuzHistogram);
        initColor=ta.getColor(R.styleable.MuzHistogram_defaultColor,initColor);
//        initHistogramWidth=ta.getDimensionPixelSize(R.styleable.Histogram_histogramWidth,initHistogramWidth);
        initHistogramWidth=ta.getInteger(R.styleable.MuzHistogram_histogramWidth,initHistogramWidth);
        histogramMinInterval=ta.getInteger(R.styleable.MuzHistogram_histogramMinInterval,histogramMinInterval);
        initTextSize=ta.getInteger(R.styleable.MuzHistogram_txtSize,initTextSize);
        initTextNameInterval=ta.getInteger(R.styleable.MuzHistogram_txtNameInterval,initTextNameInterval);
        initTextNumberInterval=ta.getInteger(R.styleable.MuzHistogram_txtNumberInterval,initTextNumberInterval);
        colours.add(ta.getColor(R.styleable.MuzHistogram_histogramColor,initColor));
        initHistogramColor=ta.getColor(R.styleable.MuzHistogram_histogramColor,initColor);
        initTextNameColor=ta.getColor(R.styleable.MuzHistogram_txtNameColor,initColor);
        initTextNumberColor=ta.getColor(R.styleable.MuzHistogram_txtNumberColor,initColor);
        initTextColor=ta.getBoolean(R.styleable.MuzHistogram_initTextColor,initTextColor);
        ta.recycle();
    }
